...
export function buildAuthoringPrompt(provider: McpProvider): string {
  const lines: string[] = [
    ...
    `  <div class="cards" data-craft-bind="repos">Loading repositories…</div>`,
    `  <button data-craft-emit="refresh">Refresh</button>`,
    `</craft>`,
    `<craft-live>{`,
    `  "subscribe":[{"channel":"github.search_repositories","args":{"query":"user:@me","perPage":30},"as":"repos","poll_s":30}],`,
    `  "actions":[{"name":"refresh","route":"direct","channel":"github.search_repositories","op":"github.search_repositories","args":{"query":"user:@me","perPage":30}}]`,
    `}</craft-live>`,
    ``,
    `# Rules`,
    ...
    `- RENDER REAL FIELDS PER ROW: inside a <template data-craft-each>, mark each value element with data-craft-field="<dot path on the row>" — the renderer fills its text from that row field. e.g. <span data-craft-field="name"></span>, <span data-craft-field="updated_at"></span>, <span data-craft-field="description"></span>. For the LIST query, pick the right args so it returns the rows the user asked for (GitHub recent repos: github.search_repositories args {"query":"user:@me sort:updated","perPage":10}; ALWAYS include sort:updated for "recently updated").`,
    `- EDITABLE BY DEFAULT: whenever you show data that a WRITE tool could update, render it as EDITABLE fields with Save buttons — DO NOT wait for the user to ask to edit. The user expects to view AND edit in one craft. Only fall back to read-only text when no matching write tool exists.`,
    `  · Showing a Notion page's blocks → ALSO render the PAGE TITLE as its own editable field at the top: an <input data-craft-input="properties.title.0.text.content" value="<current title from Resolved data>"> + a Save button whose action is {route:"direct",op:"notion.API-patch-page",args:{"page_id":"<real page id>","properties":{"title":[{"text":{"content":""}}]}}}, AND render the blocks as the editable list below. TWO editable sections in one craft: title (patch-page) + blocks (update-a-block, the per-row list).`,
    `  · Showing issues/rows → each row's title/description editable (Save via the update tool). Always pair a visible value with a way to edit+save it.`,
    ...
    `- A "Refresh" button uses data-craft-emit="refresh" + an action {name:"refresh",route:"direct",op:"<SAME tool the list subscribes to>",args:{…same…}}. Clicking it re-polls live so newly created items appear without waiting.`,
    `- WRITE: when the request implies create/send/update, add the matching [WRITE] tool as a route:"direct" action with args drawn from data-craft-input fields you include in the HTML. The product gates writes behind an approval prompt and re-polls after, so the new data shows live.`,
    ...
    `- EDITING (single entity): if the user wants to rename/edit ONE entity, author an <input data-craft-input="<argPath>"> PRE-FILLED via value="<current value from resolved data>", plus a Save button (data-craft-emit) whose action is the matching WRITE tool with the real id already in args. data-craft-input is the EXACT arg path (dots/indexes for nested). Example renaming a Notion page: input data-craft-input="properties.title.0.text.content" value="Test", action {name:"saveName",route:"direct",op:"notion.API-patch-page",args:{"page_id":"<real id>","properties":{"title":[{"text":{"content":""}}]}}}. The typed value deep-merges into args; write is approval-gated, view re-polls. Provide the full args skeleton so the merge target exists.`,
    ...
    `    (2) Inside the template put exactly one <input>/<textarea data-craft-input="<argPath relative to the write body>"> per editable field + one <button data-craft-emit="<saveAction>">Save</button>. The renderer clones the template per streamed row, pre-fills each field from the row, and stamps the row's REAL runtime id on the clone.`,
    ...
    `    Notion example:`,
    `    <div data-craft-bind="blocks">Loading blocks…<template data-craft-each data-craft-row-id="id" data-craft-type-from="type"><div class="row"><textarea data-craft-input="{type}.rich_text.0.text.content" data-craft-type-from="type" rows="2"></textarea><button data-craft-emit="saveBlock">Save</button></div></template></div>`,
    ...
    `- For a WRITE action set "hasSideEffect": true so the product asks the user to approve before it runs.`,
  ...
  const subscribe: Subscribe[] = Array.isArray(parsed.subscribe)
    ? parsed.subscribe
        ...
        .map((s) => ({
          ...
          as: String(s.as),
          poll_s: typeof s.poll_s === "number" ? s.poll_s : 30,
        }))
